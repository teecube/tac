...
This branch (*release-trigger-revision*) is a [**release trigger branch**](#release-trigger-branch) for the **T³ - TAC** project.
...
To trigger a new release with *revision* increment policy, simply [run a new pipeline](https://git.teecu.be/teecube/tac/pipelines/new) after selecting *release-trigger-revision* in the list.
That's it !
...
```shell
git clone https://git.teecu.be/teecube/tac.git
cd tac
```
...
These versions numbers can also be [edited directly in Gitlab](https://git.teecu.be/teecube/tac/edit/release-trigger-revision/release.properties).
